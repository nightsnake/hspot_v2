...
class access_pointsTable(Base):
        ...
        def __init__(self, login, password, hs_id, status, last_live, creat_date, comment, current_users, freq, type,):
            self.id = id
            self.ip = ip
            self.gw = gw
            self.login = login
        ...
        def __repr__(self):
            return "<devices_propertiesTable(%s, %s, %s, %s, %s, %s, %s, %s, %s,)>" % 
                                            (self.id, self.ip, self.gw, self.login, self.password, self.hs_id, self.comment, self.freq, self.type)
